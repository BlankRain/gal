...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.GQL, p.parseGraphQLLiteral)
...
func (p *Parser) ParseProgram() *ast.Program {
	prograpm := &ast.Program{}
	prograpm.Statements = []ast.Statement{}
	for p.curToken.Type != token.EOF {
		stmt := p.parseStatement()
		if stmt != nil {
			prograpm.Statements = append(prograpm.Statements, stmt)
		}
		p.nextToken()
	}
	return prograpm
...
func (p *Parser) parseFunctionLiteral() ast.Expression {
	ex := &ast.FunctionLiteral{
		Token: p.curToken,
	}
	if !p.expectPeek(token.LPAREN) {
		return nil
	}
	ex.Parameters = p.parseFunctionParameters()
	if !p.expectPeek(token.LBRACE) {
		return nil
	}
	ex.Body = p.parseBlockStatement()
	return ex
}
